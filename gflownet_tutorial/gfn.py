...
from gflownet_tutorial.env import RewardEnvironment
...
class GFNAgent:
    ...
    def __init__(self,
        ...
        self.data = defaultdict(None)  # {'positions': None, 'actions': None, 'rewards': None}
    ...
    @staticmethod
    def mask_and_norm_backward_actions(position, backward_probs):
    ...
    @staticmethod
    def get_last_position(trajectory):
    ...
    def sample(self, num_to_sample, explore=True, evaluate=False):
        ...
        if not evaluate:
            if self.data.get('positions') is not None:
    ...
    def clear_eval_data(self):
        ...
        self.eval_data = defaultdict(None)  # {'positions':None, 'actions':None, 'rewards':None}
